...
class GeminiLiveAudioService:
    def __init__(self, settings: Settings | None = None) -> None:
        ...
        self._cache: dict[tuple[str, Language, str, str], bytes] = {}
    ...
    async def synthesize(self, text: str, language: Language = Language.english) -> bytes:
        ...
        key = audio_cache_key(text, language, self._settings.live_audio_model, self._settings.live_audio_voice)
        if cached := self._cache.get(key):
            self._log_done(request_id, text, language, cached, time.perf_counter() - started, cache_hit=True)
            return cached
        ...
        self._log_start(request_id, text, language)
        client = genai.Client(
            ...
            speech_config=types.SpeechConfig(
                ...
                voice_config=types.VoiceConfig(
                    prebuilt_voice_config=types.PrebuiltVoiceConfig(
                        voice_name=self._settings.live_audio_voice,
                    )
                )
            ),
        )
        ...
        self._cache[key] = wav
        self._log_done(request_id, text, language, wav, time.perf_counter() - started, cache_hit=False)
        return wav

    def _log_start(self, request_id: str, text: str, language: Language) -> None:
        if not self._settings.live_audio_debug:
            return
        logger.info(
    ...
    def _log_done(
        ...
        logger.info(
            (
                "live_audio.done request_id=%s model=%s voice=%s language=%s cache_hit=%s "
                "chars=%s text_sha=%s wav_bytes=%s wav_seconds=%.2f elapsed_ms=%s"
            ),
            request_id,
            self._settings.live_audio_model,
            self._settings.live_audio_voice,
...
def audio_cache_key(text: str, language: Language, model: str, voice: str) -> tuple[str, Language, str, str]:
